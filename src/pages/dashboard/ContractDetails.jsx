...
export default function ContractDetails() {
  ...
  const handleGenerateDebt = async () => {
    ...
    const date = prompt("Fecha límite (YYYY-MM-DD):", today);
    ...
    if (amount && date) {
      ...
      try {
        await createPaymentObligation({
          contract_id: id,
          amount: parseFloat(amount), // Aseguramos que sea número
          due_date: date
          // ❌ status: "pending"  <-- BORRAMOS ESTO (El backend lo pone automático)
        });
        await loadPayments(); // Recargar lista
        alert("¡Cobro generado exitosamente!");
      } catch (error) {
        console.error("Error detallado:", error.response?.data || error); // Mejor log para debug
        // Mostramos el error real si el backend nos dice qué pasó
        const msg = error.response?.data?.detail || "Error al generar deuda";
        alert(msg);
      } finally {
        setLoading(false);
      }
